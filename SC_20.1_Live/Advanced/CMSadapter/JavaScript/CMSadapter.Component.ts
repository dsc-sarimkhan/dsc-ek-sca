...
let placeholder_views = {};

let view_ccts_to_rerender = {};
...
export = function CMSadapterComponentGenerator(application) {
    ...
    const CMSAdapterComponent = SCBaseComponent.extend({
        ...
        addContents: function addContents() {
            const self = this;
            const old_instances = _.keys(this._customContentTypesInstances);
            const async_operations = [];

            view_ccts_to_rerender = {};

            const ccts = self._sortCCTs(rawCCTs);
            ...
            _.each(ccts, function(cct: any) {
                if (_.indexOf(old_instances, cct.instance_id) >= 0) {
                    ...
                    _.each(self._customContentTypesInstances[cct.instance_id], function(
                        cct_container: any
                    ) {
                        const generator = {};
                        const container_name = cct_container.selector['data-cms-area'];
                        ...
                        const placeholders = self.getPlaceholderViews(
                            '[data-cms-area="' + container_name + '"]'
                        );

                        if (placeholders && placeholders.views) {
                            _.each(placeholders.views, function(parent_view_instance: any) {
            ...
            if (!this.application.getLayout().getCurrentView().inModal) {
                _.each(old_instances, function(old_instance_id) {
                    ...
                    _.each(self._customContentTypesInstances[old_instance_id], function(
                        cct_container: any
                    ) {
                        const container_name = cct_container.selector['data-cms-area'];
                        const placeholders = self.getPlaceholderViews(
        ...
        addContent: function addContent(
            ...
            const self = this;
            const selector = this.getPlaceholder(cct_selector);
            const container = selector
                ? this.getPlaceholderViews(self.selectorToString(selector))
                : null;
            const async_operations = [];
            ...
            if (
                ...
                !self._customContentTypes[cct_id]
            ) {
                return jQuery.Deferred().resolve();
            }
            if (
                self._customContentTypesInstances[cct_instance_id] &&
                self._customContentTypesInstances[cct_instance_id].length
            ) {
                return jQuery
                    .Deferred()
                    .reject(
                        self._createError(
                            ...
                            Utils.translate(
                                'There is already a CCT with the Instance Id $(0). Unable to add CCT: Id: $(1). CCT Instance Id: $(2). Placeholder: $(3)',
                                cct_instance_id,
                                cct_id,
                                cct_instance_id,
                                self.selectorToString(cct_selector)
                            )
                        )
                    );
            }
            ...
            _.each(container.views, function(parent_view_instance: any) {
                const cct_constructor = self._customContentTypes[cct_id].view;
                const cct_constructor_options = self._customContentTypes[cct_id].options || {};
                const cct_instance = new cct_constructor(
                    _.extend(cct_constructor_options, {
                        id: cct_id,
                        instanceId: cct_instance_id
                    })
                );
                const cct_container_instance = new CustomContentTypeContainerView({
                    innerCustomContentType: cct_instance,
                    instanceId: cct_instance_id,
                    classes: render_settings.classes
                });

                const context_data = parent_view_instance.getContextData(
                    cct_instance.getContextDataRequest()
                );
                ...
                if (cct_instance.validateContextDataRequest(context_data)) {
                    ...
                    async_operations.push(
                        self._installContent(cct_instance, cct_settings, context_data)
                    );

                    const generator = {};
                    const container_name = container.selector['data-cms-area'];
                        ...
                        jQuery
                            ...
                            .reject(
                                self._createError(
                                    ...
                                    Utils.translate(
                                        'Context for CCT not found. CCT Instance Id: $(0)',
                                        cct_instance_id
                                    )
                                )
                            )
        ...
        updateContent: function updateContent(
            ...
            const self = this;
            const cct_views_container = self._customContentTypesInstances[cct_instance_id];
            const selector = self.getPlaceholder(cct_selector);
            const async_operations = [];

            if (cct_views_container) {
                const first_cct_container = cct_views_container[0];
                        ...
                        return jQuery
                            ...
                            .reject(
                                self._createError(
                                    ...
                                    Utils.translate(
                                        'Invalid selector. The current view does not contains the specified selector: $(0)',
                                        self.selectorToString(cct_selector)
                                    )
                                )
                            );
                    }
                    const cct_id = first_cct_container.cct.id;
                ...
                _.each(cct_views_container, function(cct_container: any) {
                    const cct_container_instance = cct_container.container;
                    const parent_view_instance = cct_container_instance.parentView;
            ...
            return jQuery
                ...
                .reject(
                    self._createError(
                        'ERR_INVALID_INSTANCE_ID',
                        Utils.translate('Invalid content type instance id: $(0)', cct_instance_id)
                    )
                );
        },
        ...
        _sortCCTs: function _sortCCTs(ccts) {
            ...
            ccts.sort(function(cct_a, cct_b) {
                const cct_a_selector = self.getPlaceholder(cct_a.selector);
                const cct_b_selector = self.getPlaceholder(cct_b.selector);
        ...
        removeContent: function removeContent(cct_instance_id, destroy_call) {
            const cct_views_generator = this._customContentTypesInstances[cct_instance_id];
            const async_operations = [];

            if (cct_views_generator) {
                _.each(cct_views_generator, function(cct_generator: any) {
                    async_operations.push(
                        cct_generator.container.parentView.removeChildViewInstance(
                            cct_generator.selector['data-cms-area'],
                            cct_generator.cct.instanceId,
                            destroy_call
                        )
                    );
                });
            ...
            return jQuery
                ...
                .reject(
                    this._createError(
                        'ERR_INVALID_INSTANCE_ID',
                        Utils.translate(
                            'The specified instance id is not registered. Instance Id: $(0)',
                            cct_instance_id
                        )
                    )
                );
        ...
        getPlaceholder: function getPlaceholder(placeholder) {
            const selector_data = _.find(placeholder_views, function(obj: any) {
        ...
        registerViewForPlaceholder: function registerViewForPlaceholder(placeholders, view) {
            ...
            _.each(placeholders, function(selector) {
                const existent_selector = self.getPlaceholder(selector);

                if (existent_selector) {
                    const key_existent = self.selectorToString(existent_selector);

                    if (_.indexOf(placeholder_views[key_existent].views, view) < 0) {
                        placeholder_views[key_existent].views.push(view);
                    }
                } else {
                    const key = self.selectorToString(selector);
        ...
        unregisterViewForPlaceholders: function unregisterViewForPlaceholders(view) {
            _.each(placeholder_views, function(placeholder: any, key, selector_views_context: any) {
        ...
        getViewCctsToRerender: function getViewCctsToRerender(view) {
            if (view.template && view.template.Name) {
                const cct_generators = view_ccts_to_rerender[view.template.Name];
