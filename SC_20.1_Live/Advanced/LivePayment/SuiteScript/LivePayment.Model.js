    ...
    return SCModel.extend({
        ...
        setInvoices: function (customer_payment, result) {
            ...
            if (result.invoices.length) {
                const invoices_expanded = InvoiceModel.list({
        ...
        submit: function (data) {
            // update record
            const payment_record = this.update(
                data.internalid ? this.loadRecord(data.internalid) : this.create(data.currency_id),
                data
            );
            // save record.
            const payment_record_id = nlapiSubmitRecord(payment_record);
            // create new record to next payment.
            const new_payment_record = this.get();

            if (payment_record_id !== '0') {
                // send confirmation
                new_payment_record.confirmation = _.extend(
                    data,
                    CustomerPayment.get('customerpayment', payment_record_id)
                );
            } else {
                data.internalid = '0';
