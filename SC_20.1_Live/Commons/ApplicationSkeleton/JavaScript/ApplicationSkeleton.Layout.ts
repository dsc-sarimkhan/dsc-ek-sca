...
/// <amd-module name="ApplicationSkeleton.Layout"/>

import * as _ from 'underscore';
import '../../Utilities/JavaScript/SC.Configuration';
import * as Utils from '../../Utilities/JavaScript/Utils';
import * as jQuery from '../../Core/JavaScript/jQuery';
import { JSONObject, JSONArray } from '../../Utilities/JavaScript/Utils.Interfaces';
import { WebPage as JsonldWebPage } from '../../../node_modules/schema-dts/schema';
...
import GlobalViewsModalView = require('../../GlobalViews/JavaScript/GlobalViews.Modal.View');
import GlobalViewsBreadcrumbView = require('../../GlobalViews/JavaScript/GlobalViews.Breadcrumb.View');
import HeaderView = require('../../Utilities/JavaScript/Header.View');
import FooterView = require('../../Utilities/JavaScript/Footer.View');
...
export = BackboneView.extend({
    ...
    initialize: function(Application: any) {
        ...
        jQuery(window).on(
            ...
            _.throttle(function() {
                if (
                    Utils.getDeviceType(self.windowWidth) ===
    ...
    getColorPalette: function getColorPalette(palette_name: string) {
        ...
        _.each(this.application.getConfig('layout.colorPalette', []), function(color_item: any) {
    ...
    render: function() {
        ...
        BackboneView.prototype.render.call(this);
    ...
    updateCrumbtrail: function(pages: any) {
        if (!_.isArray(pages)) {
            pages = [pages];
        }
    ...
    appendToDom: function() {
        ...
        this.afterAppendViewPromise.done(function() {
            ...
            // show main div
            jQuery(self.container_element).css('display', 'block');
    ...
    dropdownExpander: function(e: any) {
        e.preventDefault();
        e.stopPropagation();
    ...
    showContent: function showContent(view: any, dont_scroll: boolean) {
        return this.cancelableTriggerUnsafe('beforeAppendView', view).then(
            _.bind(
                function(view: any, dont_scroll: boolean) {
                    if (!view._pagetype) {
    ...
    _showContent: function _showContent(view: any, dont_scroll: boolean): JQuery.Deferred<any> {
        ...
        if (Configuration.get('structureddatamarkup.type') === 'JSON-LD') {
            if (jQuery('[itemscope]').length || jQuery('[itemtype^="https://schema.org"]').length) {
                ...
                const promises = [
                    view.getViewJsonLd() as JSONObject,
    ...
    // @param {Backbone.View} view
    wrapModalView: function(view: any) {
    ...
    // @param {Backbone.View} view
    // @param {String} prefix
    prefixViewIds: function(view: any, prefix: string) {
        ...
        if (view instanceof Backbone.View) {
            ...
            view.$('[id]').each(function() {
                ...
                el.attr('id', function(i: any, old_id: any) {
            ...
            view.$('[for]').each(function() {
                jQuery(this).attr('for', function(i: any, old_id: any) {
                    return prefix + old_id;
                });
            });
        }
    ...
    // @param {Backbone.View} view
    addModalListeners: function(view: any, current_view: any) {
        const self = this;
    ...
    showInPush: function(view: any, options: any) {
        const self = this;
    ...
    showInModal: function(view: any, options: any) {
        return this.cancelableTriggerUnsafe('beforeAppendView', view).then(
            _.bind(this._showInModal, this, view, options)
        );
    },
    ...
    _showInModal: function(view: any, options: any) {
        ...
        view.events['mousedown [data-dismiss="modal"]'] = function(e: any) {
            e.preventDefault();
        };
        ...
        if (!view.hasRenderedInModal) {
            ...
            GlobalViewsModalView.prototype.className =
                'modal fade ' + (view.modalClass || element_id ? 'modal-' + element_id : '');
    ...
    // @return {Void}
    _renderModalView: function(view: any, current_modal_view: any) {
    ...
    // @return {Void}
    _showModalInDOM: function(view: any, options: any, current_modal_view: any, promise: any) {
        this._renderModalView(view, current_modal_view);
