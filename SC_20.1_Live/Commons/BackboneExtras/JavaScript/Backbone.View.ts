...
import SCCancelableEvents = require('../../SC/JavaScript/SC.CancelableEvents');
...
const properties = _.extend(
    {
        ...
        getCanonical: function() {
            ...
            const canonical =
                window.location.protocol + '//' + window.location.hostname + '/' + history.fragment;
        ...
        showConfirmationMessage: function(message: string, fixed: boolean) {
            // Did it in this way to avoid circular dependency
            const GlobalViewsMessageView = Utils.requireModules('GlobalViews.Message.View');
