...
define('ICart.Component', [
    ...
    let isMyJsonValid;
    const new_line_validator =
        isMyJsonValid && isMyJsonValid(CartComponentDataValidator.newLineSchema);
    const edit_line_validator =
        isMyJsonValid && isMyJsonValid(CartComponentDataValidator.editLineSchema);
    const payment_method_validator =
        isMyJsonValid && isMyJsonValid(CartComponentDataValidator.paymentMethodSchema);
    ...
    // @public @extlayer
    const icart_component = SCBaseComponent.extend({
        componentName: 'Cart',
        ...
        _validatePaymentMethod: function _validatePaymentMethod(payment_method) {
            if (payment_method_validator && !payment_method_validator(payment_method)) {
                const errors = _.reduce(
                    payment_method_validator.errors,
                    function(memo, error) {
                        return memo + ' ' + error.field + ' ' + error.message;
                    },
                    ''
        ...
        _normalizeShipMethodId: function _normalizeShipMethodId(data) {
            const shipmethod = data[0] || data[1];
        ...
        _normalizeLine: function _normalizeLine(line) {
            line.item = StoreItem
                ? StoreItem.get(
                      ...
                      line.item.itemtype || line.item.type,
                      'details'
                  )
                : line.item;
            ...
            const formatted_line = format(line, commonLineAttrs);
        ...
        _normalizePaymentMethod: function _normalizePaymentMethod(data) {
            const payment_method = Utils.deepCopy(_.first(data.paymentmethods));
            ...
            const commonGiftCertificateAttrs = [
                'internalid',
                'name',
                'creditcard',
                'ispaypal',
            ...
            const formatted_payment_method = format(payment_method, commonPaymentMethodAttrs);
        ...
        _normalizeAddPromotion: function _normalizeAddPromotion(inner_promotion) {
            const promotion = Utils.deepCopy(inner_promotion);
        ...
        _normalizeShipMethods: function _normalizeShipMethods(inner_shipmethod) {
            const ship_method = Utils.deepCopy(inner_shipmethod);
