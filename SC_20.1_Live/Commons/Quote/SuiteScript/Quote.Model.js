...
define('Quote.Model', [
    ...
    let QuoteToSalesOrderValidatorModel;
    ...
    return TransactionModel.extend({
        ...
        getTransactionRecord: function() {
            ...
            const estimate = nlapiCreateRecord('estimate', {
                recordmode: 'dynamic'
            });

            const days_to_expire = Configuration.get('quote.daysToExpire');
            if (days_to_expire !== 0) {
                estimate.setFieldValue(
                    'duedate',
                    nlapiDateToString(nlapiAddDays(new Date(), days_to_expire))
                );
            }
        ...
        getEntityStatus: function() {
            ...
            const estimate_aux = this._getQuoteStatus(this.record.getId());
        ...
        getDueDate: function() {
            // @class Quote.Model.Attributes
            const duedate = this.record.getFieldValue('duedate');
            const now = new Date().getTime();

            // @property {String} duedate
            this.result.duedate = duedate;
            // @property {Boolean} isOverdue
            this.result.isOverdue = this.isDateInterval(
                new Date(nlapiStringToDate(duedate)).getTime() - now
            );
            // @property {Boolean} isCloseOverdue
            this.result.isCloseOverdue = this.isDateInterval(
                new Date(nlapiStringToDate(duedate)).getTime() -
        ...
        mapListResult: function(result, record) {
            const duedate = record.getValue('duedate');
            const now = new Date().getTime();
            ...
            // @property {Boolean} isOverdue
            result.isOverdue = this.isDateInterval(
                new Date(nlapiStringToDate(duedate)).getTime() - now
            );
            // @property {Boolean} isCloseOverdue
            result.isCloseOverdue = this.isDateInterval(
                new Date(nlapiStringToDate(duedate)).getTime() -
                    (now + this.getDaysBeforeExpiration())
            );
            // @property {String} expectedclosedate
        ...
        postSubmitRecord: function(confirmation_result) {
            const created_salesorder =
                nlapiLookupField('estimate', confirmation_result.internalid, ['tranid']) || {};
