...
/// <amd-module name="SC.CancelableEvents"/>
...
export = {
    ...
    // @return {Void}
    cancelableOn: function cancelableOn(event_name: string, handler: any) {
    ...
    // @return {Void}
    cancelableOff: function cancelableOff(event_name: string, handler: any) {
        if (!_.isString(event_name) || !_.isFunction(handler)) {
            const error = new Error(
                'Parameter "event_name" and "handler" must be String and Function respectively'
            );
    ...
    _cancelableOff: function _cancelableOff(
        events_container_entity: any,
        event_name: string,
        handler: any
    ) {
        if (
            !events_container_entity ||
        ...
        events_container_entity._cancelableEvents[event_name] = _.reject(
            events_container_entity._cancelableEvents[event_name],
            function(event_object: any) {
    ...
    // to details the execution of the trigger's callback. If the returned jQuery.Deferred is rejected the trigger's callback wont be called
    // @return {Object} the same events_container_entity passed in as parameter
    _cancelableOn: function _cancelableOn(
        events_container_entity: any,
        event_name: string,
        handler: any
    ) {
        ...
        const event_handlers = events_container_entity._cancelableEvents[event_name];
    ...
    // @return {Void}
    cancelableDisable: function cancelableDisable(event_name: string) {
    ...
    // @param {String} event_name The name of the event to attach to
    // @return {Object} the same events_container_entity passed in as parameter
    _cancelableDisable: function _cancelableDisable(
        events_container_entity: any,
        event_name: string
    ) {
        events_container_entity._cancelableEvents = events_container_entity._cancelableEvents || {};
    ...
    // @param {String} event_name The name of the event to disable
    // @return {Void}
    cancelableEnable: function cancelableEnable(event_name: string) {
        if (!_.isString(event_name)) {
            throw {
                status: SC.ERROR_IDENTIFIERS.invalidParameter.status,
                code: SC.ERROR_IDENTIFIERS.invalidParameter.code,
    ...
    // @param {String} event_name The name of the event to attach to
    // @return {Object} the same events_container_entity passed in as parameter
    _cancelableEnable: function _cancelableEnable(
        events_container_entity: any,
        event_name: string
    ) {
        events_container_entity._cancelableEvents = events_container_entity._cancelableEvents || {};
    ...
    cancelableTrigger: function cancelableTrigger(event_name: string) {
    ...
    // @return {jQuery.Deferred} As the event handler of an event can be asynchronous (that why it use Deferrers) the invocation of the callback is async. This means that this function
    // will return Deferred to represent this asynchronous
    cancelableTriggerUnsafe: function cancelableTriggerUnsafe(event_name: string) {
        const args = Array.prototype.slice.call(arguments).slice(1);
    ...
    _cancelableTrigger: function _cancelableTrigger(
        events_container_entity: any,
        event_name: string,
        args: any[],
        safe_parameters: boolean
    ) {
        const self = this;
        ...
        const event_handler_promises = _.map(
            events_container_entity._cancelableEvents[event_name],
            function(event_handler_container: any) {
                try {
                    return event_handler_container.fn.apply(
                        null,
                        safe_parameters ? _.map(args, self._getSafeParameter) : args
                    );
                } catch (e) {
    ...
    _getSafeParameter: function _getSafeParameter(original_intent: any) {
        return _.isUndefined(original_intent)
            ? original_intent
            : JSON.parse(JSON.stringify(original_intent));
    }
};
