...
define('SC.ComponentContainer', ['underscore'], function(_) {
    ...
    return {
        ...
        getComponent: function getComponent(component_name) {
            const extensionName = this.extensionName || 'NA';
            this.component_instances[extensionName] =
                this.component_instances[extensionName] || {};
            const components = this.component_instances[extensionName];
            if (!components[component_name] && this._components[component_name]) {
                components[component_name] = new this._components[component_name](
                    extensionName
                );
