    ...
    return SCModel.extend({
        ...
        get: function() {
            ...
            if (settings_cache) {
                ...
                const allCountries = ModelsInit.session.getCountries();
                ...
                const allStates = ModelsInit.session.getStates();
                ...
                settings.campaignsubscriptions = ModelsInit.context.getFeature(
                    'CAMPAIGNSUBSCRIPTIONS'
                );
                settings.defaultSubscriptionStatus =
                    ModelsInit.context.getPreference('MARKETING_UNSUBSCRIBED_DEFAULT') === 'F';
                settings.analytics.confpagetrackinghtml = _.escape(
                    settings.analytics.confpagetrackinghtml
                );
        ...
        isPickupInStoreEnabled: function() {
            return (
                Configuration.get('isPickupInStoreEnabled') &&
        ...
        isSCISIntegrationEnabled: function() {
            return (
                Configuration.get('isSCISIntegrationEnabled') &&
        ...
        isMultiShippingRoutesEnabled: function() {
            return (
                Configuration.get('isMultiShippingEnabled') &&
        ...
        isAutoapplyPromotionsEnabled: function() {
            return (
                ModelsInit.session.getSiteSettings(['autoapplypromotionsenabled'])
                    .autoapplypromotionsenabled === 'T'
            );
        },
        // @method getTouchPoints. @returns {Object}
        getTouchPoints: function() {
            const settings = ModelsInit.session.getSiteSettings(['touchpoints', 'sitetype', 'id']);

            if (!Utils.isHttpsSupported() && settings.sitetype === 'ADVANCED') {
                settings.touchpoints.storelocator = ModelsInit.session.constructDomainBridgingUrl(
                    ModelsInit.session.getAbsoluteUrl2(
                        'checkout',
                        '/checkout.ssp?is=storelocator&n=' + settings.id
                    )
                );
            }
