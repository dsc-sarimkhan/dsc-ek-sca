    ...
    var Application = _.extend(
        {
            ...
            getEnvironment: function getEnvironment(request) {
                Configuration.set(
                    'cms.useCMS',
                    Configuration.get().cms.useCMS &&
                        ModelsInit.context.getSetting('FEATURE', 'ADVANCEDSITEMANAGEMENT') === 'T'
                );

                // Sets Default environment variables
                const siteSettings = ModelsInit.session.getSiteSettings([
                    'currencies',
                    'languages'
                ]);

                let isAlone = false;
                const { websitescope } = ModelsInit.session.getSiteSettings(['websitescope']);
                ...
                let result = {
                    ...
                    casesManagementEnabled:
                        ModelsInit.context.getSetting('FEATURE', 'SUPPORT') === 'T',
                    giftCertificatesEnabled:
                        ModelsInit.context.getSetting('FEATURE', 'GIFTCERTIFICATES') === 'T',
                    ...
                    standalone: isAlone,
                    subscriptions:
                        ModelsInit.context.getSetting('FEATURE', 'SUBSCRIPTIONBILLING') === 'T' &&
                ...
                if (result.availableHosts.length) {
                    const pushLanguage = function(language) {
                        result.availableLanguages.push(
                            _.extend({}, language, available_languages_object[language.locale])
                        );
                    };
                    const pushCurrency = function(currency) {
                        result.availableCurrencies.push(
                            _.extend({}, currency, available_currencies_object[currency.code])
                        );
                    };

                    for (let i = 0; i < result.availableHosts.length; i++) {
                        const host = result.availableHosts[i];
                        if (host.languages && host.languages.length) {
                            // looks for the language match
                            for (let n = 0; n < host.languages.length; n++) {
                                const language = host.languages[n];
                                ...
                                if (language.host === result.shoppingDomain) {
                                    ...
                                    var available_languages_object = _.object(
                                        _.pluck(result.availableLanguages, 'locale'),
                        ...
                        if (result.currentHost) {
                            ...
                            /// ///////////////////////////////////////////////////////////
                            var available_currencies_object = _.object(
                                _.pluck(result.availableCurrencies, 'code'),
                ...
                /// ///////////////////////////////////
                const currency_codes = _.pluck(result.availableCurrencies, 'code');
                ...
                if (!result.currentLanguage) {
                    const shopper_preferences = ModelsInit.session.getShopperPreferences();
                    const shopper_locale = shopper_preferences.language.locale;
                    const locales = _.pluck(result.availableLanguages, 'locale');

                    if (
                        request.getParameter('lang') &&
                        ~locales.indexOf(request.getParameter('lang'))
                    ) {
                        result.currentLanguage = _.find(result.availableLanguages, function(
                            language
                        ) {
            ...
            getPermissions: function getPermissions() {
                ...
                const purchases_returns_permissions = [
                ...
                return {
                    transactions: {
                        ...
                        tranSalesOrdFulfill: ModelsInit.context.getPermission(
                            'TRAN_SALESORDFULFILL'
                        ),
            ...
            sendError: function(e) {
                ...
                let content = Application.processError(e);
            ...
            getPaginatedSearchResults: function(options) {
                ...
                const range_start = page * results_per_page - results_per_page;
                const range_end = page * results_per_page;
                const do_real_count = _.any(columns, function(column) {
                ...
                if (!do_real_count || options.column_count) {
                    const column_count =
                        options.column_count || new nlobjSearchColumn('internalid', null, 'count');
                    const count_result = nlapiSearchRecord(record_type, null, filters, [
                        column_count
                    ]);
                ...
                if (
                    ...
                    const search = nlapiCreateSearch(record_type, filters, columns).runSearch();
            ...
            getAllSearchResults: function(record_type, filters, columns) {
                const search = nlapiCreateSearch(record_type, filters, columns);
                search.setIsPublic(true);

                const searchRan = search.runSearch();
                let bolStop = false;
                let intMaxReg = 1000;
                let intMinReg = 0;
                let result = [];
                ...
                while (!bolStop && ModelsInit.context.getRemainingUsage() > 10) {
                    ...
                    const extras = searchRan.getResults(intMinReg, intMaxReg);
            ...
            addFilterSite: function(filters) {
                const search_filter_array = this.getSearchFilterArray();

                search_filter_array.length &&
                    filters.push(
                        new nlobjSearchFilter('website', null, 'anyof', search_filter_array)
                    );
            },
            ...
            addFilterItem: function(filters) {
                ...
                search_filter_array.length &&
                    filters.push(
                        new nlobjSearchFilter('website', 'item', 'anyof', search_filter_array)
                    );
            },
            ...
            getSearchFilterArray: function() {
                ...
                const filter_site = Configuration.get('filterSite');
                let search_filter_array = [];

                // Validate if: MULTISITE, site_id, filter_site and also if filter_site is different of 'all'
                if (
                    ModelsInit.context.getFeature('MULTISITE') &&
                    site_id &&
                    filter_site.option &&
...
const invalidItemsFieldsAdvancedName = {
    // @property {Number} status
    status: 500,
    // @property {String} code
