...
define('Configuration', ['Utils', 'underscore', 'SC.Models.Init', 'Console'], function(
    Utils,
    _,
    ModelsInit
) {
    ...
    function adaptValuestoOldFormat(configurationObj) {
        ...
        if (
            ...
            _.each(configurationObj.multiDomain.hosts.languages, function(language) {
                const storedHost = _.find(configurationObj.hosts, function(host) {
                ...
                if (!storedHost) {
                    configurationObj.hosts.push({
                        ...
                        currencies: _.filter(
                            configurationObj.multiDomain.hosts.currencies,
                            function(currency) {
    ...
    function Configuration(config) {
        const effectiveDomain = config.domain || ModelsInit.session.getEffectiveShoppingDomain();
        const effectiveSiteId =
            config.siteId || ModelsInit.session.getSiteSettings(['siteid']).siteid;
        ...
        if (Utils.recordTypeExists('customrecord_ns_sc_configuration')) {
            const config_key = effectiveDomain
                ? effectiveSiteId + '|' + effectiveDomain
                : effectiveSiteId + '|all';
            const search = nlapiCreateSearch(
                'customrecord_ns_sc_configuration',
                [new nlobjSearchFilter('custrecord_ns_scc_key', null, 'is', config_key)],
                [new nlobjSearchColumn('custrecord_ns_scc_value')]
            );
            const result = search.runSearch().getResults(0, 1000);
            const configuration =
                (result.length &&
            ...
            if (configurationExtensions[effectiveDomain]) {
                this.configurationProperties = deepExtend(
                    this.configurationProperties,
                    configurationExtensions[effectiveDomain]
                );
            }
