...
import AddressesCollection = require('../../Address/JavaScript/Address.Collection');
...
const TranactionModel: any = Backbone.Model.extend({
    ...
    initialize: function(attributes) {
        ...
        this.on('change:addresses', function(model, addresses) {
            if (model.previous('addresses') instanceof AddressesCollection) {
                ...
                model
                    .get('addresses')
                    .reset(addresses instanceof AddressesCollection ? addresses.models : addresses);
            } else {
    ...
    addPayment: function addPayment(payment_method) {
        ...
        // Sets it as primary
        (payment_method) ? payment_method.set('primary', true) : {}; //check if method is not undefined
