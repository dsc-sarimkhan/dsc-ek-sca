    ...
    return SCModel.extend({
        ...
        list: function(data) {
            ...
            const records = _.map(
                ...
                function(record) {
                    const selected_currency = Utils.getCurrencyById(record.getValue('currency'));
                    ...
                    const result = {
                        ...
                        amount_formatted: Utils.formatCurrency(
                            record.getValue(self.amountField),
        ...
        getTransactionRecord: function(record_type, id) {
            ...
            console.log('load record', {type: record_type, id: id});
            var r =  nlapiLoadRecord(record_type, id);
            console.log('load record', r.getFieldValue('indernalid'));
            return r;
        },
        ...
        get: function(record_type, id) {
            ...
            if (record_type && id) {
                ...
                if (this.result.currency)
                    this.result.selected_currency = Utils.getCurrencyById(
                        this.result.currency.internalid
                    );
        ...
        getCreatedFrom: function() {
            ...
            const created_from_internalid = this.record.getFieldValue('createdfrom');
            let record_type = '';
        ...
        setTransactionBodyCustomFields: function() {
            const self = this;
            const customFieldsId = CustomFieldsUtils.getCustomFieldsIdToBeExposed(this.recordType);

            _.each(this.data.options, function(value, id) {
                // only set a custom field to the model if was exposed in the configuration
                if (
                    _.find(customFieldsId, function(configFieldId) {
        ...
        getRecordPromocodes: function() {
            ...
            const promocode = this.record.getFieldValue('promocode');
            ...
            for (let i = 1; i <= this.record.getLineItemCount('promotions'); i++) {
                if (
                    this.record.getLineItemValue('promotions', 'applicabilitystatus', i) !==
                    'NOT_APPLIED'
                ) {
                    this.result.promocodes.push({
        ...
        getRecordPaymentMethod: function() {
            ...
            const ccnumber = this.record.getFieldValue('ccnumber');
        ...
        getRecordSummary: function() {
            const selected_currency_symbol = this.result.selected_currency ?
            ...
            this.result.summary =
                // @class Transaction.Model.Get.Summary
                {
                    ...
                    subtotal_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('subtotal'),
                        selected_currency_symbol
                    ),
                    ...
                    taxtotal_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('taxtotal'),
                        selected_currency_symbol
                    ),
                    ...
                    tax2total_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('tax2total'),
                        selected_currency_symbol
                    ),
                    ...
                    handlingcost_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('althandlingcost'),
                        selected_currency_symbol
                    ),
                    ...
                    discounttotal_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('discounttotal'),
                        selected_currency_symbol
                    ),
                    ...
                    // @property {String} discountrate_formatted
                    discountrate_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('discountrate'),
                        selected_currency_symbol
                    ),
                    ...
                    giftcertapplied_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('giftcertapplied'),
                        selected_currency_symbol
                    ),
                    ...
                    total_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('total'),
                        selected_currency_symbol
                    )
                };
        ...
        getLines: function() {
            ...
            for (let i = 1; i <= this.record.getLineItemCount('item'); i++) {
                if (
                    this.record.getLineItemValue('item', 'itemtype', i) === 'Discount' &&
                    this.record.getLineItemValue('item', 'discline', i)
                ) {
                    const discline = this.record.getLineItemValue('item', 'discline', i);

                    //For discounts in quotes
                    line_id = (self.result.internalid === 'tempid' ? self.result.quote_id : self.result.internalid) + '_' + discline;

                    amount = Math.abs(
                        parseFloat(this.record.getLineItemValue('item', 'amount', i))
                    );
                    ...
                    this.result.lines[line_id].discount_name = this.record.getLineItemValue(
                        'item',
                        'item_display',
                        i
                    );
                } else {
                    const rate = Utils.toCurrency(this.record.getLineItemValue('item', 'rate', i));
                    const item_id = this.record.getLineItemValue('item', 'item', i);
                    const item_type = this.record.getLineItemValue('item', 'itemtype', i);

                    amount = Utils.toCurrency(this.record.getLineItemValue('item', 'amount', i));

                    const tax_amount =
                        Utils.toCurrency(this.record.getLineItemValue('item', 'tax1amt', i)) || 0;
                    ...
                    this.result.lines[line_id] = {
                        ...
                        options: self.parseLineOptions(
                            this.record.getLineItemValue('item', 'options', i)
                        ),
                        // @property {String} shipaddress
                        shipaddress: this.record.getLineItemValue('item', 'shipaddress', i) ?
                            this.result.listAddresseByIdTmp[
                                  this.record.getLineItemValue('item', 'shipaddress', i)
                            ] : null,
            ...
            const preloaded_items = this.preLoadItems(_.values(items_to_preload));
            const selected_currency_symbol = self.result.selected_currency ?
            ...
            _.each(this.result.lines, function(line) {
                ...
                line.tax_amount_formatted = Utils.formatCurrency(
                    line.tax_amount,
                    selected_currency_symbol
                );
                line.discount_formatted = Utils.formatCurrency(
                    line.discount,
                    selected_currency_symbol
                );
        ...
        loadItemsWithStoreItem: function(items_to_preload) {
            ...
            _.each(this.result.lines, function(line) {
                if (line.item) {
                    const item = self.storeItem.get(line.item, line.type);
        ...
        loadItemsWithSuiteScript: function(items_to_query) {
            ...
            if (items_to_query.length > 0) {
                ...
                const inactive_items_search = Application.getAllSearchResults(
                    'transaction',
                    filters,
                    columns
                );
        ...
        getRecordShippingMethods: function() {
            ...
            if (this.record.getLineItemCount('shipgroup') <= 0) {
                ...
                self.addShippingMethod({
                    ...
                    rate_formatted: Utils.formatCurrency(
                        this.record.getFieldValue('shipping_rate'),
                        self.result.selected_currency ?
                        self.result.selected_currency.symbol :
                        undefined
                    ),
            ...
            for (let i = 1; i <= this.record.getLineItemCount('shipgroup'); i++) {
                self.addShippingMethod({
        ...
        getTransactionType: function(ids) {
            ...
            const columns = [new nlobjSearchColumn('recordtype')];

            if (ids && ids.length) {
                _.each(
                    Application.getAllSearchResults('transaction', filters, columns) || [],
                    function(record) {
        ...
        setAddress: function(prefix, address_id, other_address_name) {
            ...
            if (address_id) {
                if (!this.hasCurrentCustomerAddress(address_id)) {
                    const old_address_model = _.find(this.data.addresses, {
                        internalid: address_id
                    });
                    const old_address_id = address_id;
                ...
                _.each(this.data.lines, function(line) {
                    ...
                    _.each(line.options, function(option) {
                        if (option.cartOptionId && option.value && option.value.internalId) {
                            self.record.setCurrentLineItemValue(
                                'item',
                                option.cartOptionId,
        ...
        removeAllItemLines: function() {
            const items_count = this.record.getLineItemCount('item');
