...
export function highlightKeyword(text: string, keyword: string): string {
    ...
    keyword = String(keyword).trim().replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, '\\$&');
...
interface BXSliderOptions extends bxSliderOptions {
    forceStart?: boolean;
}

export function initBxSlider($element: JQuery, options: BXSliderOptions) {
