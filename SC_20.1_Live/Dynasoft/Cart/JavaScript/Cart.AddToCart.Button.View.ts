...
export = BackboneView.extend({
    ...
    addToCart: function addToCart(e) {
        ...
        if (!this.model.isNew() && this.model.get('source') === 'cart') {
            ...
            if (self.modelsToAdd.length > 0) {
                this.upsellItemsAdd(self.modelsToAdd, () => {
                    self.showConfirmation(cart_promise,line,self.options.application);
                });
            }else{
                self.showConfirmation(cart_promise,line,self.options.application);
            }
    ...
    showConfirmation : function showConfirmation(cart_promise, line, application){
        ...
        collection.fetch({
            ...
            success: function (response) {
                _.each(array, function (item: any) {
                    ...
                    const selectedItem = response.get(item.model._id)?response.get(item.model._id):item.model;
