...
import './SC.Checkout.Configuration.Steps.Standard';
import './SC.Checkout.Configuration.Steps.OPC';
import './SC.Checkout.Configuration.Steps.BillingFirst';
import * as Utils from '../../../Commons/Utilities/JavaScript/Utils';
import * as jQuery from '../../../Commons/Core/JavaScript/jQuery';

import BaseConfiguration = require('../../../Advanced/SCA/JavaScript/SC.Configuration');
...
const configuredCheckoutStepsName =
    checkoutStepsMap[BaseConfiguration.get('checkoutApp.checkoutSteps')] ||
    BaseConfiguration.get('checkoutApp.checkoutSteps');

let config_promise: any = jQuery.Deferred();
if (configuredCheckoutStepsName) {
    Utils.requireModules(
        [configuredCheckoutStepsName],
        config_promise.resolve,
        config_promise.reject
    );
} else {
    console.log(
        'Configured Checkout steps AMD module not found for ' +
            configuredCheckoutStepsName +
            '. Using standard flow'
    );
    Utils.requireModules(
