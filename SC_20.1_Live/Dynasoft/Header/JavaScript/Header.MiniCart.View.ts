...
export = BackboneView.extend({
    ...
    getContext: function () {
        ...
        _.each(this.model.get('lines').models, function (line: any) {
            ...
            if (line.get('internalid') != undefined) {
                totalQuantity += line.get('quantity');

            }
        });
       var filterArray=  this.model.get('lines').models.filter(function(model){
            return model.get('internalid') != undefined
        })
        if(filterArray.length != this.model.get('lines').length){

        this.model.set('lines',filterArray);

    }
        // if (this.itemsInCart != totalQuantity) {
        //     // this.render();
