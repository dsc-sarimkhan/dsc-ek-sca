...
define('LiveOrder.ServiceController', [
    ...
    'SiteSettings.Model',
    'SC.Models.Init'
], function (ServiceController, LiveOrderModel, SiteSettings, ModelsInit) {
    ...
    return ServiceController.extend({
        ...
        post: function () {
            ...
            // Submit the order
            const confirmation = LiveOrderModel.submit();
            // Get the new order
        ...
        put: function () {
            this.setShopperCurrency();
            if(this.data.fulfillment_choice){
                var orderItem = _.find(this.data.lines, function(line){
                    return line.freeGift == false;
                });
                LiveOrderModel.updateLineModels(orderItem, this.data.fulfillment_choice);
            }else{
                LiveOrderModel.update(this.data);
            }
            return LiveOrderModel.get();
