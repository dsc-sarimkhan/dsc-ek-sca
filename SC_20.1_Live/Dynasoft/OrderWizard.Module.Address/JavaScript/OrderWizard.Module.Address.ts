...
const OrderWizardModuleAddress: any = WizardModule.extend({
    ...
    submit: function() {
        ...
        if (this.addressView) {
            ...
            if(!zip || !city){return;}
            jQuery("#address-error").remove();  //remove previous error first
            validateAddress(zip, city, (cityNames, state) => {
                ...
                if(isValid){
                    ...
                    jQuery('.global-views-message-error').text('Verifying Shipping Adress')
                    ...
                    jQuery(".order-wizard-step-button-continue").prop('disabled', true);
    ...
    isValid: function() {
        ...
        var selected_address = addresses && addresses.get(this.model.get(this.manage));
        if(this.manage === 'billaddress' && selected_address === undefined){
            var billaddress = window.localStorage.getItem('billaddress') || ""; //DSC added stored address to be available for afterpay or zippay
            selected_address = addresses.get(billaddress);
        }
