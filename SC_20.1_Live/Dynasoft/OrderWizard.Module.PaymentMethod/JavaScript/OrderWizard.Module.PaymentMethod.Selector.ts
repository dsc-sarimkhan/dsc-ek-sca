...
import OrderWizardModel = require('../../../Advanced/OrderWizard/JavaScript/OrderWizard.Model');
...
const OrderWizardModulePaymentMethodSelector: any = WizardModule.extend({
    ...
    initialize: function(options) {
        ...
        if (!options.disableExternalPaymentMethods) {
            ...
            this.externalPaymentMethods = external_payment_methods; // DSC: saving to global variable
            ...
            if (external_payment_methods.length) {
                _.each(external_payment_methods, function(payment_method: any) {
                    ...
                    if(payment_method.name && payment_method.name.toLowerCase() === 'zippay'){ 
                        self.model.set('zippay_image',payment_method.imagesrc[0]); 
                        return ; 
                    } //DSC: skip zippay to hide from website
                    if(payment_method.name && payment_method.name.toLowerCase() === 'afterpay'){ 
                        self.model.set('afterpay_image',payment_method.imagesrc[0]);
                        return; 
                    } //DSC: skip afterpay to hide from website
                ...
                if (order_wizard_paymentmethod_others_module_tpl && !_.isEmpty(self.external)) {
                    this.modules.push({
    ...
    render: function() {
        ...
        if (!this.selectedModule) {
            ...
            if (selected_payment) {
                ...
                selected_type = 'creditcard';//selected_type = 'invoice' //set payment type to credit card;
            }
    ...
    // @method isValid
    isValid: function() {
        if(DSCUtils.getURLParam('result') && DSCUtils.getURLParam('result').toLowerCase() === 'approved'){
            this.model.set('customPayMethod','zippay')
        }
        //DSC: added for zippay
        if(this.model.get('customPayMethod') === 'zippay'){
            let zipPayMethod = _.find(this.externalPaymentMethods, function(method: any){
                return method.name.toLowerCase() == 'zippay';
            });
            if(zipPayMethod){
                this.model.set('customPaymentMethod', zipPayMethod);
                return true;
            }
        }//end
        if(DSCUtils.getURLParam('status') && DSCUtils.getURLParam('status').toLowerCase() === 'success'){
            this.model.set('customPayMethod','afterpay')
        }
        //DSC: added for zippay
        if(this.model.get('customPayMethod') === 'afterpay'){
            let afterPayMethod = _.find(this.externalPaymentMethods, function(method: any){
                return method.name.toLowerCase() == 'afterpay';
            });
            if(afterPayMethod){
                this.model.set('customPaymentMethod', afterPayMethod);
                return true;
            }
        }//end
