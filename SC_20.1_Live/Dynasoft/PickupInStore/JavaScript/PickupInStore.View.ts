...
import Location = require('../../../Advanced/Location.SCA/JavaScript/Location');
...
const PickupInStoreView: any = BackboneView.extend({
    ...
    initialize: function initialize(options) {
        ...
        //DSC customization
        // var defaultLocation = this.getDefaultShowroomLocation();
        
        const location_id = 401;//this.model.get('location').get('internalid') || Cookies.get('myStore');
    ...
    getLocationStock: function getLocationStock() {
        return 1;//DSC customization:bypass quantity check for location pickup
    ...
    getDefaultShowroomLocation: function(){
        var ekConfigurationModel = new EKConfigurationModel();
        ekConfigurationModel.fetch({
            data:{'key':'default-pickup-location'},
            async:false
        });
        return ekConfigurationModel;
    },
    ...
    getContext: function getContext() {
        ...
        return {
            ...
            hideOOSMessage: (this.model.get('item') !== undefined) ? this.model.get('item').get('custitem_dsc_hide_oos_message') : this.model.get('custitem_dsc_hide_oos_message')
        };
    }
});
