...
var ui = {
    selectToken: function (deploy, cb) {
        if (deploy.info.authID) {
            ...
            var tokens_1 = OAuth1.getAllTokens();
            var tokenList = Object.keys(tokens_1).map(function (tokenName) { return ({
                name: tokenName + " - Account " + tokens_1[tokenName].account,
                value: tokenName
            }); });
            tokenList.unshift({ value: 'newToken', name: 'New token' });
            if (!tokenList.length) {
                log('Remember to have TOKEN-BASED AUTHENTICATION feature enabled in the target account.');
            }
            inquirer.prompt([{
                    type: 'list',
                    name: 'token',
        ...
        if (deploy.info.website && deploy.info.domain) {
            ...
            inquirer
                .prompt([
                {
                    ...
                    choices: deploy.websitesAndDomains.websites
                        .map(function (ws) {
                        return { name: ws.displayname, value: ws.id };
                    })
                        .sort(alphabeticSort)
                }
			])
			.then(function (answers) {
                ...
                var domains = _.filter(deploy.websitesAndDomains.websiteDomainData, function (data) {
                    return data.ws === deploy.info.website;
                });
                inquirer
                    .prompt([
                    {
                        ...
                        choices: domains
                            .map(function (domain) {
                            return { name: domain.domain, value: domain.domain };
                        })
                            .sort(alphabeticSort)
                    }
                ], function (answers) {
                    deploy.info.domain = answers.domain;
                    var domainInfo = _.find(deploy.websitesAndDomains.websiteDomainData, function (data) {
                        return (data.domain === deploy.info.domain &&
                            data.ws === deploy.info.website);
                    });
    ...
    rollback: function (deploy, cb) {
        if (deploy.rollback_revision) {
            ...
            var choices = deploy.revisions
                ...
                var name = "" + new Date(revision.deployment_time)
                    .toISOString()
                    .slice(0, 19)
                    .replace('T', ' ');
                ...
                longest_1 = longest_1 < name.length ? name.length : longest_1;
                return {
                    name: name,
                    value: revision
                };
            });
            choices.push(new inquirer.Separator(new Array(longest_1 + 1).join('-')));
            inquirer
                .prompt([
                {
                    type: 'list',
    ...
    token: function (deploy, cb) {
        if (deploy.info.authID) {
            ...
            inquirer.prompt([promp_config]).then(function (answers) {
                ...
                log('Continue the authentication process in the prompted browser.');
                cb(null, deploy);
            });
        }
    },
    targetHostingFolder: folderInquirer({
        ...
        choices: function (deploy) {
            return deploy.target_folders
                .map(function (hosting) {
                return { name: hosting.name, value: hosting };
            })
                .sort(alphabeticSort);
        }
    }),
    targetPublisherFolder: folderInquirer({
        ...
        choices: function (deploy) {
            return deploy.folder_inquirer.target_hosting_folder.hosting.publishers
    ...
    targetSSPFolder: folderInquirer({
        ...
        choices: function (deploy) {
            ...
            applications = applications.filter(function (application) {
                var exists = false;
                applications.forEach(function (app) { exists |= app.name + "2" === application.name; });
                return !exists;
            });
            return applications
