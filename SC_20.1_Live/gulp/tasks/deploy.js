...
function runDeploy(files, options, cb)
{
	var tasks = _.map(files, function(file)
	{
		return function(cb)
		{
			...
			return gulp.src(process.gulp_dest + '*/**', {allowEmpty: true})
...
function deployVersion (cb)
{
	function copy_version(cb)
	{
		return gulp.src('./version.txt', {allowEmpty: true})
	...
	{
		return copy_version(cb);
	}

	var exec = require('child_process').exec;
	exec(`p4 changes -m 1 ${path.join(process.gulp_init_cwd, '/../../../src')}/...#have`, function(err, stdout)
	{
		var version = stdout;
		if (err || !version || version.toString().match('Error'))
		{
...
if(args._.indexOf('deploy') >= 0)
{
	...
	else //for default always we create a "ssp_libraries_output" and if is to test we compile this.
	{
        ...
        if (!fs.existsSync(process.gulp_dest_deploy))
        {
            fs.mkdirSync(process.gulp_dest_deploy);
        }

		fs.writeFileSync(dstOutput, '//this test is not available in this deploy', {flag: 'w'});
