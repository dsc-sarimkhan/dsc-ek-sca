<div class="cart-detailed">
	...
	{{#unless selectedFulfillmentChoice}} 
		<div data-view="GlobalMessageNoFulfillment"></div>
	{{/unless}}
	...
	<div class="cart-detailed-body">
		<section class="{{#if showLines}}cart-detailed-left {{else}}cart-detailed-empty{{/if}}">
			...
			<br>
			{{#if showLines}}
			<div id='type' class="cart-summary-container" style="text-align: center;">
				<h3 class="cart-summary-title">Fulfillment Choice: </h3>
				<span>
					<input type="radio" data-action="changeFulfillment" id="ship-order" class = "radioButton" name="order-status" value="ship" {{#ifEquals selectedFulfillmentChoice 'ship'}}checked{{/ifEquals}}>
					<label for="ship">Ship</label>
					<input type="radio" data-action="changeFulfillment" id="pick-up-order" class = "radioButton" name="order-status" value="pickup" style="margin-left: 30px;" {{#ifEquals selectedFulfillmentChoice 'pickup'}}checked{{/ifEquals}} {{#if itemOutOfStock}}disabled{{/if}}>
					<label for="pick-up">	Pickup at <a>Evakool HQ</a></label>
					{{#if itemOutOfStock}}<p class="out-of-stock-info">
						<span class="product-line-stock-icon-out"><i></i></span>
						<span>{{notForPickupMessage}}</span>
					</p>{{/if}}
				</span>
			</div>
			{{/if}}
			<br>
			{{#if upsell}}
			<div data-view="Cart.Purchased.Items"></div>
			{{/if}}
			{{#if showLines}}
			<div class="cart-detailed-proceed-to-checkout-container">
			{{#unless upsell}}
			<a class="cart-detailed-proceed-to-checkout" data-action="sticky" href="/cart?upsell=true">
				{{translate 'Continue'}}
			</a>
			<br>
			{{/unless}}
