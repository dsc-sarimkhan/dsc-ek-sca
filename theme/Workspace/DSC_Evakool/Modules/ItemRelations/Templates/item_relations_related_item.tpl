<div itemprop="itemListElement" data-item-id="{{itemId}}" data-track-productlist-list="{{track_productlist_list}}"
	...
	{{#if showIncludeCheckbox}}
		<div>
			<input type="text" name="model_{{itemId}}" value="{{model}}" class="{{itemId}}"  hidden>
			<input type="checkbox" class="include_upsell" name="include_upsell" value="{{itemId}}" /> Add to Cart
		</div>
	{{/if}}
	...
	<form id="addToCartForm">
		<input type="text" name="quickaddSearch" value="{{model._sku}}" hidden>
		<input type="text" name="quantity" value="1" hidden>
		<button class="btn btn-primary submitBtn" id="{{model._sku}}" type="submit">Add item</button>
		<br>
	</form>
	{{/if}}
</div>
