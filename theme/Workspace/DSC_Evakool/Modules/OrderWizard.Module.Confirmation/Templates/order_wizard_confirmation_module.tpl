<div class="order-wizard-confirmation-module alert fade in">
	...
	<p class="order-wizard-confirmation-module-body" name="orderNumber">{{translate 'Your order number is'}}
		<strong>
			...
			<a href="#" data-touchpoint="customercenter"
				data-hashtag="#/purchases/view/salesorder/{{orderId}}">#{{confirmationNumber}}</a>.
			{{else}}
			#{{confirmationNumber}}
			{{/if}}
		</strong>
	...
	<p class="order-wizard-confirmation-module-body">
		{{translate 'We received your order and will process it right away.'}}</p>
	{{#if additionalConfirmationMessage}}
	<p class="order-wizard-confirmation-module-body" data-type="additional-confirmation-message">
		{{additionalConfirmationMessage}}</p>
	{{/if}}
	<a class="order-wizard-confirmation-module-continue" href="{{continueURL}}" {{#if touchPoint}}data-touchpoint="home"
		{{/if}} data-hashtag="#/">{{translate 'Continue shopping'}}</a>
	<!-- DOWNLOAD AS PDF -->
...
<script>
	$(document).ready(function () {	
		fbq('track', 'Purchase', {
		currency: "AUD",
		value: "{{model.confirmation.summary.total}}"
	});
	});
	// 	console.log("Model->",model);
	// 	console.log("this->",this);
	// console.log("model->",model.confirmation.summary.total);
